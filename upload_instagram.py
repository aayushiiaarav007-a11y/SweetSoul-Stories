#!/usr/bin/env python3
"""
SweetSoul Stories - OPTIONAL Instagram uploader (CLI).
...
log = logging.getLogger("sweetsoul.upload.instagram")
...
def _caption(entry):
    title = entry.get("title", "A SweetSoul Story")
    hashtags = get_cfg(
        "youtube.hashtags",
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Upload SweetSoul reels to Instagram (optional).")
