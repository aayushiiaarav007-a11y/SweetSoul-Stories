#!/usr/bin/env python3
"""
SEO self-check for SweetSoul Stories.
...
def sample_shorts(n):
    ...
    for s in scripts:
        meta = seo.build_metadata(s.title, s.text, s.keywords)
...
def check_shorts(rows, check):
    ...
    for i, m in enumerate(rows):
        ...
        check.ok("subscrib" in desc.lower() or "follow" in desc.lower(),
...
def check_longform(rows, check):
    ...
    for i, m in enumerate(rows):
        ...
        check.ok("MoralTales" not in desc,
                 f"{tag}: description still uses the mismatched MoralTales brand")
...
# --------------------------------------------------------------------------
def main(argv=None):
    parser = argparse.ArgumentParser(description="Verify SweetSoul SEO metadata.")
    ...
    print("=" * 70)
    print(f"SweetSoul SEO self-check - {mode} - {args.count} samples")
    print("=" * 70)
